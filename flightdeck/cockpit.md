...
先取得 Actions 运行 `34321214614` 的失败步骤日志；本地质量检查已通过，且 npm 已存在 `0.1.0`，若为发布冲突则递增版本并创建新标签。全局 Fraq CLI 已升级到 `1.0.1`，但 `my-fraq-app` 仍需单独处理本地 CLI/Fraq 核心兼容性，再验证 npm 包版 `doudizhu` 的群聊指令和图片发送。
